...
class PaymentService
{
    ...
    public function pay($order)
    {
        ...
        $returnUrl = source_base_url('/#/order/' . $order['trade_no']);
        $gatewayReturn = env('SLOTH_GATEWAY_PAYMENT_RETURN_URL');
        if ($gatewayReturn) {
            $separator = str_contains($gatewayReturn, '?') ? '&' : '?';
            $returnUrl = $gatewayReturn . $separator . 'order_no=' . urlencode($order['trade_no']);
        }
